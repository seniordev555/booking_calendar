...
function sendEmailUpdateBooking(booking, callback) {
    var receivers = [];
    booking.populate('owner personnel.adr_mixer personnel.shared_users', function (err, booking) {
        receivers = [];
        ...
        if (booking.personnel.adr_mixer) {
            receivers.push(booking.personnel.adr_mixer)
        }
...
function sendEmailConfirmationToOwner(booking, callback) {
    var receivers = [];
    booking.populate('owner personnel.adr_mixer', function (err, booking) {
        if (booking.owner && booking.owner.email) {
            receivers.push(booking.owner);
        }
        ...
        async.mapSeries(receivers, function (receiver, nextReceiver) {
            if (!receiver.email || ! receiver.wantReceiveEmailBookingCreated()) {
                return nextReceiver();
            }
...
function sendMailForSharedUsers(users, booking, callback)
{
    ...
    async.eachSeries(users, function (user, nextUserCb) {
        ...
        _.defaultsDeep(data, {
            sender: booking.owner.fullname || booking.owner.email.substr(0, booking.owner.email.indexOf("@")),
...
function sendEmailDeleteBooking(booking, callback) {
    var receivers = [];
    receivers = receivers.concat(booking.personnel.shared_users);
    if (booking.owner && booking.owner.email) {
        receivers.unshift(booking.owner);
    ...
    async.mapSeries(receivers, function (receiver, nextReceiver) {
        ...
        var template = path.join('views', 'emails', 'booking-deleted');
        var data = { booking: booking.toJSON() };
        _.defaultsDeep(data, {
            sender: booking.owner.fullname || booking.owner.email.substr(0, booking.owner.email.indexOf("@")),
            receiver: receiver.fullname || '',
...
module.exports = {
    ...
    "sendEmailDeleteBooking": sendEmailDeleteBooking
};
